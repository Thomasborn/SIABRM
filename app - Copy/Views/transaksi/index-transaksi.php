    ...
    <div class="flex-none w-full max-w-full px-3">
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div class="flex items-center p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                <h6 class="font-bold text-slate-700 text-xl dark:text-white">Daftar Transaksi Sewa</h6>
                <form class="ml-auto">
                ...
                <div>
                    <a href="/transaksi/entry" class="px-8 py-2 mb-4 ml-5 font-bold leading-normal text-center text-white align-middle transition-all ease-in bg-blue-500 border-0 rounded-lg shadow-md cursor-pointer text-xs tracking-tight-rem hover:shadow-xs hover:-translate-y-px active:opacity-85">Tambah</a>
                </div>
            </div>
                        ...
                        <tbody class="card-content">
                            ...
                            foreach ($result as $value) : ?>
                                <tr class="card">
                                    ...
                                    <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                        <p id="tanggal_mulai" class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80"><?= date('d M Y', strtotime($value['tanggal_mulai_sewa'])) ?></p>
                                        <p id="tanggal_mulai" class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80"><?= date('g : i A', strtotime($value['tanggal_mulai_sewa'])) ?></p>
                                    </td>
                                    <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                        <p id="tanggal_akhir" class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80"><?= date('d M Y', strtotime($value['tanggal_akhir_sewa'])) ?></p>
                                        <p id="tanggal_akhir" class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80"><?= date('g : i A', strtotime($value['tanggal_akhir_sewa'])) ?></p>
                                    </td>
                                    <td class="p-2 text-sm leading-normal text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                        <?php if ($value['tanggal_kembali'] != '0000-00-00 00:00:00') : ?>
                                    ...
                                    <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent flex items-center">
                                        <form action="/transaksi/detail/<?= $value['id_transaksi_sewa'] ?>">
                                        ...
                                        <form action="/transaksi/<?= $value['id_transaksi_sewa'] ?>" method="post" class="">
                                            <?= csrf_field() ?>
