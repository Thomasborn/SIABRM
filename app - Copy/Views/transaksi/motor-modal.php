                    ...
                    <div class="flex-auto px-0 pt-0 pb-2 mt-4">
                        <div class="p-0 overflow-x-auto">
                            <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                                ...
                                <tbody class="card-content2" style="display: none;">
                                    <?php foreach ($motor as $value) : ?>
                                        <tr class="card2">
                                            ...
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <button role="button" onclick="pilihMotor('<?= $value['id_motor'] ?>','<?= $value['nama_motor'] ?>','<?= $value['merk'] ?>','<?= $value['harga_sewa'] ?>','<?= $value['no_polisi'] ?>')" class="bg-emerald-500 rounded-lg px-4 py-2 text-white text-xs font-semibold leading-tight dark:text-white dark:opacity-80" role="button">
                            ...
                            <div class="mt-4 mb-6 mx-6 flex items-center">
                                ...
                                <?= $this->include('pagination/my-pagination2') ?>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    function pilihMotor(id, name, merk, harga, nopolisi) {
        $('#id_motor').val(id);
        $('#nama_motor').val(name);
        $('#merk').val(merk);
        $('#harga_sewa').val(harga);
        $('#no_polisi').val(nopolisi);
