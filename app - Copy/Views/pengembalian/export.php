                            ...
                            <div class="overflow-x-auto rounded-t-xl">
                                <table class="w-full mb-4 align-top border-collapse border-gray-200 text-slate-500 dark:border-white/40">
                                    <thead class="text-white align-bottom bg-slate-700 dark:bg-slate-700">
                                        <tr>
                                            <th scope="col" class="px-2 py-3 font-semibold text-left capitalize bg-transparent border-b border-solid shadow-none tracking-none whitespace-nowrap border-b-gray-200 dark:border-white/40 dark:text-white">Item</th>
                                    ...
                                    <tbody class="border-t-2">
                                        <tr>
                                            <td class="p-2 text-left border-b border-slate-300 whitespace-nowrap dark:border-white/40 dark:text-white/60">
                                                <div>
                                                    <?= $result['nama_motor'] ?>
                                                </div>
                                                <div>
                                            ...
                                            <td id="jumlah_hari" class="p-2 pl-6 text-center border-b border-slate-300 whitespace-nowrap dark:border-white/40 dark:text-white/60"></td>
                                            <td class="p-2 pl-6 text-right border-b border-slate-300 whitespace-nowrap dark:border-white/40 dark:text-white/60" colspan="2"><?= number_to_currency($result['harga_sewa'], 'IDR', 'id_ID', 2) ?></td>
                                            <td id="harga_sewa" class="hidden"><?= $result['harga_sewa'] ?></td>
                                            <td id="subtotal" class="p-2 pl-6 text-right border-b border-slate-300 whitespace-nowrap dark:border-white/40 dark:text-white/60"></td>
                                        </tr>
                                        <?php foreach ($detail as $value) : ?>
                                            <tr>
                                                <td class="p-2 text-left first-letter:capitalize border-b whitespace-nowrap dark:border-white/40 dark:text-white/60"><?= $value['nama_kerusakan'] ?></td>
                                                <td class="p-2 text-center pl-6 border-b whitespace-nowrap dark:border-white/40 dark:text-white/60">1</td>
                                                <td class="p-2 text-right pl-6 border-b whitespace-nowrap dark:border-white/40 dark:text-white/60" colspan="2"><?= number_to_currency($value['denda'], 'IDR', 'id_ID', 2) ?></td>
                                                <td class="p-2 text-right pl-6 border-b whitespace-nowrap dark:border-white/40 dark:text-white/60"><?= number_to_currency($value['denda'], 'IDR', 'id_ID', 2) ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                        <?php if ($overcharge != 0) : ?>
                                            <tr>
                                                <td class="p-2 text-left first-letter:capitalize border-b whitespace-nowrap dark:border-white/40 dark:text-white/60">Overcharge</td>
                                                <td class="p-2 text-center pl-6 border-b whitespace-nowrap dark:border-white/40 dark:text-white/60"><?= $hours ?> jam</td>
                                                <td class="p-2 text-right pl-6 border-b whitespace-nowrap dark:border-white/40 dark:text-white/60" colspan="2"><?= number_to_currency($harga, 'IDR', 'id_ID', 2) ?></td>
                                                <td class="p-2 text-right pl-6 border-b whitespace-nowrap dark:border-white/40 dark:text-white/60"><?= number_to_currency($overcharge, 'IDR', 'id_ID', 2) ?></td>
                                            </tr>
                                        <?php endif; ?>
                                        <td id="overcharge" class="hidden"><?= $overcharge ?></td>
                                        <p id="total_denda" class="hidden"><?= $denda['denda'] ?></p>
                                    </tbody>
                                    <tfoot class="border-t border-solid">
                                        <tr>
                                            ...
                                            <th id="total_bayar" class="p-2 pl-6 mt-0 mb-2 text-xl text-right font-semibold leading-snug bg-transparent border-b shadow-none whitespace-nowrap text-slate-700 dark:border-white/40 dark:text-white/60" colspan="1"></th>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
                ...
                <div class="border-black/12.5 mt-6 rounded-b-2xl border-t-0 border-solid p-6 md:mt-12">
                    <div class="flex flex-wrap -mx-3">
                        ...
                        <div class="w-full max-w-full px-3 mt-4 lg:flex-0 shrink-0 md:mt-0 md:text-right lg:w-7/12">
                            <button onclick="printDiv()" type="button" value="click" class="inline-block px-5 py-2.5 mb-0 text-sm font-bold leading-normal text-right text-white align-middle transition-all ease-in bg-blue-500 border-0 rounded-lg shadow-md cursor-pointer hover:-translate-y-px active:opacity-85 hover:shadow-xs tracking-tight-rem bg-150 bg-x-25 lg:mt-24">Print</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    document.getElementById('subtotal').innerHTML = rupiah(total_harga);
    ...
    var overcharge = document.getElementById('overcharge').innerHTML;
    console.log(overcharge);
    const total_bayar = Math.abs(total_harga + (total_denda * 1) + (overcharge * 1));
    document.getElementById('total_bayar').innerHTML = rupiah(total_bayar);
