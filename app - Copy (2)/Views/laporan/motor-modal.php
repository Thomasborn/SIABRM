                                    ...
                                    <?php foreach ($motor as $value) : ?>
                                        <tr>
                                            ...
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent grid grid-rows-3">
                                                <button role="button" onclick="pilihMotor('<?= $value['id_motor'] ?>','<?= $value['nama_motor'] ?>','<?= $value['merk'] ?>','<?= $value['harga_sewa'] ?>','<?= $value['no_polisi'] ?>')" class="bg-emerald-500 rounded-lg px-2 py-1 text-white text-xs font-semibold leading-tight dark:text-white dark:opacity-80" role="button">
...
<script>
    function pilihMotor(id, name, merk, harga, nopolisi) {
        $('#id_motor').val(id);
        $('#nama_motor').val(name +" plat ("+ nopolisi+")");
        $('#merk').val(merk);
