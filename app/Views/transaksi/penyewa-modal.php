<div id="penyewa-modal" class="hidden fixed top-0 left-0 w-screen h-screen flex items-center z-sticky bg-black/50">
    <div class=" w-9/10 h-4/5 mx-auto">
                                    ...
                                    <?php foreach ($penyewa as $value) : ?>
                                        <tr class="card">
                                            ...
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent grid grid-rows-3">
                                                <button role="button" onclick="pilihPenyewa('<?= $value['id_penyewa'] ?>','<?= $value['nama_penyewa'] ?>','<?= $value['alamat_domisili'] ?>','<?= $value['hp_penyewa'] ?>','<?= $value['universitas'] ?>')" class="bg-emerald-500 rounded-lg px-2 py-1 text-white text-xs font-semibold leading-tight dark:text-white dark:opacity-80" role="button">
...
<script>
    function pilihPenyewa(id, name, alamat, hp, univ) {
        $('#id_penyewa').val(id);
        $('#nama_penyewa').val(name);
